...
pub async fn get_courses_for_tutor_db(pool: &PgPool, tutor_id: i32) -> Vec<Course> {
    let course_rows = sqlx::query!(r#"select course_id, tutor_id, course_name, posted_time from ezy_course_c4 where course_id = $1"#, tutor_id)
    .fetch_all(pool)
    .await
    .unwrap();
...
pub async fn get_course_details_db(pool: &PgPool, tutor_id: i32, course_id: i32) -> Course {
    ...
    let course_row = sqlx::query!("SELECT tutor_id, course_id, course_name, posted_time FROM ezy_course_c4 where tutor_id = $1 and course_id = $2", tutor_id, course_id)
    .fetch_one(pool)
    .await
    .unwrap();
    // Execute query
